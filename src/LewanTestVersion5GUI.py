...
def inverse_kinematics(
    ...
    """Solve inverse kinematics with an improved initial guess."""
    
    # Calculate initial base rotation toward the target
    initial_theta1 = np.arctan2(target_pos[1], target_pos[0])
    
    # Initial guess: base rotation + neutral positions for other joints
    theta_rad = np.array([initial_theta1, 0.0, 0.0, 0.0, 0.0])
    print(f"Initial theta_rad: {theta_rad}") # Debug statement
    ...
    for _ in range(max_iter):
        ...
        print(f"Computed positions: {positions}") # Debug statement
        theta_rad = positions_to_radians(positions)
    else:
        print(f"Warning: IK did not converge. Error: {np.linalg.norm(target_pos - p):.2f} mm")
...
class RobotArmGUI:
    ...
    def create_input_frame(self):
        ...
        tk.Button(frame, text="Random movements", command=self.random_movements).grid(row=5, column=0, columnspan=3, pady=5)
    ...
    def create_ik_equation_frame(self):
        ...
        # Simplified text representation of damped least-squares IK
    ...
    def random_movements(self):
        ...
        # Generate random theta_rad within joint limits
        theta_rad = np.array([
            np.random.uniform(-np.pi/2, np.pi/2),  # Joint 1
            np.random.uniform(-np.pi/2, np.pi/2),  # Joint 2
            np.random.uniform(-np.pi/2, np.pi/2),  # Joint 3
            np.random.uniform(-np.pi/2, np.pi/2),  # Joint 4
            np.random.uniform(-np.pi, np.pi)       # Joint 5
        ])
        ...
        self.x_entry.insert(0, f"{random_pos[0]:.1f}")
        self.y_entry.delete(0, tk.END)
        self.y_entry.insert(0, f"{random_pos[1]:.1f}")
        self.z_entry.delete(0, tk.END)
        self.z_entry.insert(0, f"{random_pos[2]:.1f}")
        
        # Randomly select gripper state
        gripper_state = np.random.choice(["open", "closed"])
        self.gripper_var.set(gripper_state)
